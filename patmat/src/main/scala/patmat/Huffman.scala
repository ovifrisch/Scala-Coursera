...
object Huffman {
   ...
   */
    ...
    * */
    def times(chars: List[Char]): List[(Char, Int)] = {
      ...
      def new_acc(lst : List[(Char, Int)], Elem : Char) : List[(Char, Int)] = {
        lst match {
          case List() => List((Elem, 1))
          case (Elem, count) :: tail => (Elem, count + 1) :: tail
          case head :: tail => head :: new_acc(tail, Elem)
        }
      }
      def helper(chars: List[Char], acc: List[(Char, Int)]) : List[(Char, Int)] = {
        chars match {
          case List() => acc
          case head :: tail => helper(tail, new_acc(acc, head))
        }
      }
   ...
   */
    def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
      def helper(freqs: List[(Char, Int)]): List[Leaf] = {
        freqs match {
          case Nil => Nil
          case (char, count) :: tail => Leaf(char, count) :: makeOrderedLeafList(tail)
        }
      }
   ...
   */
    def combine(trees: List[CodeTree]): List[CodeTree] = {
      ...
      def insert_(tree : CodeTree, trees : List[CodeTree]) : List[CodeTree] = {
        trees match {
          case Nil => List(tree)
          case head :: tail => {
            if (weight(tree) <= weight(head)) tree :: trees
            else head :: insert_(tree, tail)
          }
        }
      }
      ...
      if (trees.length < 2) trees
        ...
        insert_(makeCodeTree(trees(0), trees(1)), trees.tail.tail)
      }
    }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def traverse(tree2: CodeTree, bits: List[Bit]): List[Char] = tree2 match {
   ...
   */
    def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
      ...
      def helper(tree2: CodeTree, text: List[Char], code: List[Bit]) : List[Bit] = {
        if (text == Nil) code.reverse
        else {
          tree2 match {
            case Leaf(c,_) => helper(tree, text.tail, code)
            case Fork(left, right, _, _) => {
              if (Huffman.chars(left).contains(text.head)) helper(left, text, 0 :: code)
              else helper(right, text, 1 :: code)
            }
          }
        }
      }
   ...
   */
    def mergeCodeTables(a: CodeTable, b: CodeTable): CodeTable = {
      def prepend(bit : Bit) (code : Code): Code = (code._1, bit :: code._2)
